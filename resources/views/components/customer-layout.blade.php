...
<body class="min-h-screen bg-white font-sans text-slate-800 antialiased">
    <!-- HEADER — sengaja dibikin identik sama landing page (PublicLayout.vue)
         supaya customer yang login gak berasa pindah ke "aplikasi lain". -->
...
=======
            <nav class="hidden gap-8 md:flex">
                <a href="/" class="text-sm font-medium text-slate-600 transition hover:text-brand-600">Beranda</a>
                <a href="{{ route('customer.kirim') }}"
                   class="text-sm font-medium transition {{ request()->routeIs('customer.kirim*') ? 'text-brand-600' : 'text-slate-600 hover:text-brand-600' }}">
                    Kirim Paket
                </a>
                <a href="/lacak" class="text-sm font-medium text-slate-600 transition hover:text-brand-600">Lacak Paket</a>
                <a href="{{ route('customer.dashboard') }}"
                   class="text-sm font-medium transition {{ request()->routeIs('customer.dashboard') ? 'text-brand-600' : 'text-slate-600 hover:text-brand-600' }}">
                    Dashboard Saya
>>>>>>> 7f212d9de6c10c5f1227a5e90633dd57e257b7c5
                </a>
            </nav>
...
=======
                <span class="hidden text-sm text-slate-500 sm:block">Hai, {{ explode(' ', auth('customer')->user()->name)[0] }}</span>
                <form method="POST" action="{{ route('customer.logout') }}">
                    @csrf
                    <button type="submit"
                            class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 transition hover:border-rose-300 hover:text-rose-600">
                        Keluar
>>>>>>> 7f212d9de6c10c5f1227a5e90633dd57e257b7c5
                    </button>
                </form>
    ...
    <div class="mx-auto max-w-6xl px-5 py-3">
        ...
        @unless (auth('customer')->user()->hasVerifiedEmail())
            <div class="mb-4 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-700">
                Email kamu belum diverifikasi.
                <a href="{{ route('verification.notice') }}" class="font-semibold underline">Verifikasi sekarang</a>
>>>>>>> 7f212d9de6c10c5f1227a5e90633dd57e257b7c5
...
=======
    ...
    <!-- FOOTER — sama persis kayak landing page -->
        ...
        <div class="mx-auto grid max-w-6xl gap-10 px-5 py-14 md:grid-cols-4">
            ...
            <div>
                ...
                <ul class="mt-3 space-y-2 text-sm text-slate-500">
                    <li><a href="{{ route('customer.kirim') }}" class="hover:text-brand-600">Kirim Paket</a></li>
